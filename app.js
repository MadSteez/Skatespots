  ...
  spotListEl.innerHTML = spots
    .map((spot) => {
      const thumb = spot.images && spot.images[0]
        ? `<img class="spot-card__thumb" src="${spot.images[0]}" alt="" loading="lazy">`
...
$("spotForm").addEventListener("submit", async (e) => {
  ...
  if (previewItems.length === 0) return showToast("Add at least one photo.", { error: true });
...
refreshAll().then(() => {
  ...
  if (allSpots.length === 0 && !localStorage.getItem("skatespots_config")) {
